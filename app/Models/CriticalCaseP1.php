...
class CriticalCaseP1 extends Model
{
    protected $table="critical_cases";
    ...
    {
        return $this->hasMany(CriticalCheck::class,'critical_check_id');
    }
    ...
    public function getChecklistItemStatusAttribute()
    {
        ...
        foreach($this->checklist->items as $item){
            ...
            $daily_check_item = CriticalCheck::where('critical_check_id',$this->id)->where('checklist_item_id',$item->id)->where('status','Okay')->first();
            if($daily_check_item){
