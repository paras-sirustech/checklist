...
namespace Ooredoo\CriticalCasesP1\Http\Controllers;

use App\Models\CriticalCaseP1;
use App\Models\CriticalCheck;
...
class CriticalCasesP1Controller extends BaseController
{
    ...
    public function startDailyCheck(Request $request)
    {
        ...
        if ($user->access_level == 'Admin') {
            $shop = Shop::with('checklist.criticalItems')->where('id', request('shop'))->first();
        } else {
            $shop = Shop::with('checklist.criticalItems')->where('id', request('shop'))->whereHas('ip_addresses', function ($query) {
                $query->where('ip_address', request()->ip());
            })->first();
        }
        ...
        $shop->checklist->items = $shop->checklist->criticalItems;
        $critical_check = CriticalCaseP1::where('shop_id', $shop->id)->whereDate('checking_date', request('date'))->where('is_submission_complete', 1)->first();
        if ($critical_check) {
            $redirect_to = config('app.url') . '/app/resources/daily-critical-checks/' . $critical_check->id;
    ...
    public function saveDailyCheckItem(Request $request)
    {
        ...
        $daily_check = CriticalCaseP1::firstOrCreate([
            'shop_id' => request('shop'),
            'checking_date' => request('date'),
        ]);

        $daily_check_item = CriticalCheck::firstOrCreate([
            'critical_check_id' => $daily_check->id,
    ...
    public function createDailyCheckIssue(Request $request)
    {
        ...
        $checkingDate = !empty($daily_check_item->daily_check) ? $daily_check_item->daily_check->checking_date : Carbon::parse(request('date'));
        $support_ticket = SupportTicket::firstOrCreate([
            'status' => 'Open',
            'created_by' => $user->id,
            'daily_check_id' => $daily_check_item->critical_check_id,
            'daily_check_item_id' => $daily_check_item->id,
            'checklist_item_id' => $daily_check_item->checklist_item_id,
            'shop_id' => $daily_check_item->daily_check ? $daily_check_item->daily_check->shop_id : request('shop'),
            'checking_date' => $checkingDate,
        ]);

        $support_ticket->subject = 'Daily Check - ' . $checkingDate->toFormattedDateString() . ' - ' . $daily_check_item->checklist_item_name;
    ...
    public function fileDailyCheck(Request $request)
    {
        ...
        $daily_check = CriticalCaseP1::where('shop_id', $shop->id)->whereDate('checking_date', request('date'))->first();
        ...
        if ($daily_check->items->count() < $shop->checklist->criticalItems->count()) {
        ...
        $redirect_to = config('app.url') . '/app/resources/daily-critical-checks/' . $daily_check->id;
