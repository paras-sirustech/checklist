    ...
    <div v-else>
      <div class="flex border-b border-40">
        ...
        <div class="py-6 px-8 w-1/2">
          <select v-model="criticalCasesItemStatus" class="w-full form-control form-select" @change="setOption">
      ...
      <div v-if="showCreateIssueForm" class="bg-30">
        ...
        <div class="flex border-b border-40">
          ...
          <div class="py-6 px-8 w-4/5">
            <button type="button" :disabled="!this.issueDescription" class="btn btn-default btn-primary inline-flex items-center relative" @click="createCriticalCasesIssue">
  ...
  methods: {
    notifyParent() {
      this.$emit("selectItemOption", {
        id: this.item.id,
        value: this.criticalCasesItemStatus
      });
    },
    setOption(event) {
      if (this.criticalCasesItemStatus == "Okay") {
        this.showCreateIssueForm = false;
        this.submitCriticalCasesItem();
      }

      if (this.criticalCasesItemStatus == "Not Okay") {
        this.showCreateIssueForm = true;
      }
    },
    createCriticalCasesIssue: function() {
      this.loading = true;
      ...
      formData.append("status", this.criticalCasesItemStatus);
      ...
      axios
        .post("/nova-vendor/critical-cases-p1/item-create-issue", formData, {
          headers: {
    ...
    submitCriticalCasesItem: function() {
      this.loading = true;
      axios
        .post("/nova-vendor/critical-cases-p1/save-item", {
          checklist_item_id: this.item.id,
          checklist_item_name: this.item.name,
          shop: this.shop.id,
          date: this.date,
          status: this.criticalCasesItemStatus
        })
        .then(
          function(response) {
            this.loading = false;
